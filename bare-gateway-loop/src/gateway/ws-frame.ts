// ──────────────────────────────────────────────────────────────────────────
// Minimal RFC6455 WebSocket *data frame* codec (the bare loop previously did
// only the upgrade handshake). Client→server frames are masked; server→client
// frames are not. Supports text/close/ping/pong and 7/16/64-bit lengths.
// ──────────────────────────────────────────────────────────────────────────
...
// Parse ONE frame from the front of `buf`. Returns null if a full frame isn't
// buffered yet (caller accumulates more bytes and retries).
export function parseFrame(buf: Buffer): { frame: WsFrame; rest: Buffer } | null {
  ...
  for (let i = 0; i < len; i++) {
    payload[i] = mask ? raw[i] ^ mask[i & 3] : raw[i];
  }
