// ──────────────────────────────────────────────────────────────────────────
// VERBATIM — origin: openclaw/src/gateway/auth-resolve.ts (whole file)
//
// STEP 2 (auth layer): turn raw gateway.auth config (+ optional override + env
// + tailscale mode) into a ResolvedGatewayAuth { mode, token, password,
// allowTailscale, trustedProxy }. This is the function the real gateway calls
// to decide how every WS/HTTP connection will be authenticated.
//
// Mode precedence (verbatim logic below):
//   override.mode → config.mode → (password set ? password) → (token set ? token) → token
//
// Only the GatewayTailscaleMode/TrustedProxy type imports + the credentials
// + secret-ref helpers are needed; all are provided as siblings in this tree.
// ──────────────────────────────────────────────────────────────────────────
