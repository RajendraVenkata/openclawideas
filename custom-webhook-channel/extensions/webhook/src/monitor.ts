// ──────────────────────────────────────────────────────────────────────────
// The inbound webhook ("read" path): an HTTP server on its own port that
// accepts POST <path>, verifies the shared secret, ACKS immediately (202), and
// then routes the message to the agent asynchronously.
// ──────────────────────────────────────────────────────────────────────────

import { createServer, type IncomingMessage } from "node:http";
import type { InboundHandler } from "openclaw/channel-sdk.js";
...
export type MonitorWebhookResult = {
...
export async function monitorWebhookProvider(
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    // Shared-secret check (skipped when no secret is configured = local mode).
    if (secret) {
    ...
    // ACK immediately — the reply is delivered out-of-band (async), like a real
    // chat webhook.
